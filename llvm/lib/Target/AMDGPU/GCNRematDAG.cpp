...
unsigned RematDAG::findAndMoveBestRemat(ArrayRef<unsigned> Remats,
  ...
  remat(BestRegIdx, *InsertPos->getParent(), InsertPos, Reg.getDefReg(),
        *OrigMI);
...
unsigned RematDAG::rematRegInRegion(unsigned RegIdx, unsigned UseRegion,
  ...
  if (auto RegionUses = RegUses.find(UseRegion); RegionUses != RegUses.end()) {
    ...
    NewReg.Uses.try_emplace(UseRegion, RegionUses->getSecond());
...
unsigned RematDAG::rematerialize(unsigned RootIdx) {
  ...
  for (const auto &[UseRegion, RegionUses] : Uses) {
    if (UseRegion != DefRegion)
      rematRegInRegion(RootIdx, UseRegion, RegionUses.FirstMI);
  }
  return Regs.size() - NumRegsBeforeRemat;
}

unsigned RematDAG::rematerializeToRegion(unsigned RootIdx, unsigned UseRegion,
                                         DependencyData &DD) {
  RematReg::RegionUses RegionUses = getReg(RootIdx).Uses.at(UseRegion);
  rematerializeToPos(RootIdx, getReg(RootIdx).Uses.at(UseRegion).FirstMI, DD);
  unsigned NewRootIdx = Regs.size() - 1;
  RematReg &NewRootReg = Regs.back();

  // The users of the original register in the using region become the users of
  // the new register in its defining region.
  auto &RegUses = Regs[RootIdx].Uses;
  for (MachineInstr *UserMI : RegionUses.Users)
    substituteUserReg(RootIdx, NewRootIdx, *UserMI);
  NewRootReg.Uses.try_emplace(UseRegion, RegionUses);
  RegUses.erase(UseRegion);
  deleteRegIfUnused(RootIdx);
  return NewRootIdx;
}

void RematDAG::rematerializeToPos(unsigned RootIdx,
                                  MachineBasicBlock::iterator InsertPos,
                                  DependencyData &DD) {
  // Pre-allocate space for the new registers in the vector to avoid
  // invalidating references during rematerialization.
  Regs.reserve(Regs.size() + DD.Dependencies.size() + 1);
  LLVM_DEBUG({
  ...
  std::function<unsigned(unsigned)> RematToPos = [&](unsigned RematIdx) {
    // Create dependencies for the new register.
    SmallVector<RematReg::Dependency, 2> NewDeps;
    for (const RematReg::Dependency &Dep : Regs[RematIdx].Dependencies) {
      unsigned NewDepIdx;
      auto NewDep = DD.DepMap.find(Dep.RegIdx);
      if (NewDep != DD.DepMap.end()) {
        // We already have the version of the dependency we want to use.
        NewDepIdx = NewDep->second;
      } else {
        // We must rematerialize requested dependencies in def-use order.
        NewDepIdx = DD.Dependencies.contains(Dep.RegIdx)
                        ? RematToPos(Dep.RegIdx)
                        : Dep.RegIdx;
        DD.DepMap.insert({Dep.RegIdx, NewDepIdx});
      }
      NewDeps.emplace_back(Dep.MOIdx, NewDepIdx);
    }
    return createReg(RematIdx, UseRegion, std::move(NewDeps), InsertPos);
  };

  // First rematerialize dependencies, then the root register.
  for (unsigned DepIdx : DD.Dependencies) {
    if (auto NewDep = DD.DepMap.find(DepIdx); NewDep == DD.DepMap.end()) {
      DD.DepMap.insert({DepIdx, RematToPos(DepIdx)});
      LLVM_DEBUG(rdbgs() << "Rematerialized dependency: " << printID(DepIdx)
                         << " to " << printID(DD.DepMap.at(DepIdx)) << '\n');
    }
  }
  RematToPos(RootIdx);
  LLVM_DEBUG(--CallDepth);
...
void RematDAG::partiallyRollbackReg(unsigned RegIdx) {
  ...
  remat(RegIdx, MBB, InsertPos, OldDefReg, *ModelReg.DefMI);
...
void RematDAG::updateLiveIntervals() {
  // Update intervals for rematerializable registers.
  ...
  for (unsigned RegIdx : LISUpdates) {
    ...
    for (unsigned MOIdx : Reg.UnrematableOprds) {
      Register UnrematReg = Reg.DefMI->getOperand(MOIdx).getReg();
      if (!SeenUnrematRegs.insert(UnrematReg).second)
        continue;
      LIS.removeInterval(UnrematReg);
      LIS.createAndComputeVirtRegInterval(UnrematReg);
      LLVM_DEBUG(dbgs() << "  Re-computed interval for unrematable register "
                        << llvm::printReg(
                               UnrematReg, &TRI,
                               Reg.DefMI->getOperand(MOIdx).getSubReg(), &MRI)
                        << '\n');
    }
  }
  LISUpdates.clear();
...
void RematDAG::collectRematRegs(unsigned DefRegion) {
  ...
  for (auto MI = Bounds.first; MI != Bounds.second; ++MI) {
    ...
    if (!DefReg.isVirtual() || !MRI.hasOneDef(DefReg)) {
      LLVM_DEBUG(
          dbgs()
          << "  -> Eliminated: non-virtual or more than one definition\n");
      continue;
    }
...
RematDAG::createReg(unsigned RegIdx, unsigned UseRegion,
  ...
  remat(NewRegIdx, *InsertPos->getParent(), InsertPos, NewDefReg, *Reg.DefMI);
...
unsigned RematDAG::findAvailableRematInRegion(unsigned RegIdx, unsigned Region,
                                              SlotIndex AvailableAt) const {
  const RematReg &Reg = getReg(RegIdx);
  auto It = Reg.Remats.find(Region);
  if (It == Reg.Remats.end())
    return NoReg;
  ArrayRef<unsigned> RegionRemats = It->getSecond();
  assert(!RegionRemats.empty() && "empty region remats");

  SlotIndex BestSlot;
  unsigned BestRegIdx = NoReg;
  for (unsigned RematRegIdx : RegionRemats) {
    const RematReg &RematReg = getReg(RematRegIdx);
    if (!RematReg.DefMI) {
      // The rematerialization could be dead if it was itself fully
      // rematerialized or rolled back.
      continue;
    }
    SlotIndex RematRegSlot =
        LIS.getInstructionIndex(*RematReg.DefMI).getRegSlot();
    if (RematRegSlot < AvailableAt &&
        (BestRegIdx == NoReg || RematRegSlot > BestSlot)) {
      BestSlot = RematRegSlot;
      BestRegIdx = RematRegIdx;

      // Check that the register's is actually available at the position. Don't
      // rely on a live-range check as live ranges can become stale during
      // rematerialization. The register must be available up to its last use in
      // the region, so we can just check whether that is later than the target
      // position.
      // if (auto It = RematReg.Uses.find(Region); It != RematReg.Uses.end()) {
      //   // FIXME: Re-enable this kind of check once we have a better idea of
      //   how
      //   // to do it; it's too conservative at the moment.

      //   // SlotIndex AliveTo =
      //   LIS.getInstructionIndex(*It->getSecond().LastMI);
      //   // if (AliveTo.getRegSlot(true) >= AvailableAt)
      //   //   return RematRegIdx;
      // }
    }
  }
...
RematReg::getInsertPos(MachineBasicBlock::iterator InsertPos,
  ...
  if (auto RegionUses = Uses.find(UseRegion); RegionUses != Uses.end()) {
    MachineBasicBlock::iterator EarliestUser = RegionUses->getSecond().FirstMI;
...
void RematReg::RegionUses::addUser(MachineInstr *NewUser,
  ...
  SlotIndex UserSlot = LIS.getInstructionIndex(*NewUser);
  if (UserSlot < LIS.getInstructionIndex(*FirstMI))
    FirstMI = NewUser;
  if (LIS.getInstructionIndex(*LastMI) < UserSlot)
    LastMI = NewUser;
}
...
void RematReg::RegionUses::eraseUser(MachineInstr *DeletedUser,
  ...
  if ((FirstMI == DeletedUser || LastMI == DeletedUser) && !Users.empty()) {
    auto UsersIt = Users.begin();
    SlotIndex MinSlot = LIS.getInstructionIndex(**UsersIt);
    SlotIndex MaxSlot = MinSlot;
    FirstMI = LastMI = *UsersIt;

    for (auto E = Users.end(); UsersIt != E; ++UsersIt) {
      SlotIndex UserSlot = LIS.getInstructionIndex(**UsersIt);
      if (UserSlot < MinSlot) {
        MinSlot = UserSlot;
        FirstMI = *UsersIt;
      }
...
Printable RematDAG::printChain(unsigned RootIdx) const {
  return Printable([&, RootIdx](raw_ostream &OS) {
    ...
    for (const auto &[RegIdx, Depth] : Regs) {
      ...
      OS << Shift << Sep << printReg(RegIdx, /*SkipRegions=*/Depth) << '\n';
    }
...
Printable RematDAG::printID(unsigned RegIdx) const {
  return Printable([&, RegIdx](raw_ostream &OS) {
    ...
    OS << llvm::printReg(DefReg, &TRI, SubIdx, &MRI) << ")[" << Reg.DefRegion
       << "]";
  });
}
...
Printable RematDAG::printReg(unsigned RegIdx, bool SkipRegions) const {
  return Printable([&, RegIdx, SkipRegions](raw_ostream &OS) {
    const RematReg &Reg = getReg(RegIdx);
    if (!SkipRegions) {
      OS << printID(RegIdx) << " [" << Reg.DefRegion;
      if (!Reg.Uses.empty()) {
        assert(Reg.DefMI && "dead register cannot have uses");
        const LiveInterval &LI = LIS.getInterval(Reg.getDefReg());
        ...
        auto It = Reg.Uses.begin();
        OS << It->first;
        while (++It != Reg.Uses.end())
          OS << "," << It->first;
      }
      OS << "] ";
    }
