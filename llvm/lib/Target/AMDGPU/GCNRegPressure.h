...
struct GCNRegPressure {
  ...
  /// Accounts for a pressure change induced by \p Reg's mask going from \p
  /// PrevMask to \p NewMask.
  void inc(unsigned Reg, LaneBitmask PrevMask, LaneBitmask NewMask,
           const MachineRegisterInfo &MRI);

  /// Accounts for a pressure change of \p DiffNumRegs 32-bit registers
  /// (negative values indicate pressure decreases) in register class \p RC.
  /// \p AccountTuple indicates, for a tuple RC, whether the tuple register of
  /// concern is being (partiallu) defined for the first time or killed
  /// entirely; the value is irrelevant for non-tuple RCs.
  void inc(int DiffNumRegs, bool AccountTuple, const TargetRegisterClass *RC,
           const MachineRegisterInfo &MRI);
