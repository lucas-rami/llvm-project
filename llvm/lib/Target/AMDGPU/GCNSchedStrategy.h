...
private:
  ...
  struct RematReg {
    ...
    SmallDenseMap<unsigned, SmallVector<MachineInstr *, 4>> Users;
    struct DependentReg {
      Register Reg;
      unsigned SubIdx;
      PointerIntPair<RematReg *, 1, bool> RematAndAvail;

      DependentReg(Register Reg, unsigned SubIdx, RematReg *Remat,
                   bool Available)
          : Reg(Reg), SubIdx(SubIdx), RematAndAvail(Remat, Available) {}
    };
    SmallVector<DependentReg, 2> Deps;
    ...
    RematReg(MachineInstr *DefMI, unsigned DefRegion, unsigned DefFrequency,
             GCNScheduleDAGMILive &DAG)
        : CandReg(DefMI, DefRegion, DefFrequency, DAG) {}

    bool analyzeUsers(GCNScheduleDAGMILive &DAG,
                      const DenseMap<unsigned, RematReg *> &Remats,
                      const DenseMap<MachineInstr *, unsigned> &MIRegion,
                      ArrayRef<uint64_t> RegionFreq);

    void notifyRemat(std::pair<unsigned, MachineInstr *> From,
                     std::pair<unsigned, MachineInstr *> To);

    bool checkIfRematable();
    ...
    /// TODO \p Returns true if all the uses in \p InstToRemat defined at \p
    /// OriginalIdx are live at \p RematIdx. This only checks liveness of
    /// virtual reg uses.
    bool isRegAvailableAtUsers(Register Reg, unsigned SubIdx,
