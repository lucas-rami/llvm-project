...
public:
  ...
  getLiveRegsForRegionIdx(unsigned RegionIdx) const {
    assert(IdxToInstruction.contains(RegionIdx));
    MachineInstr *Key = IdxToInstruction.at(RegionIdx);
    if (auto It = RegionLiveRegMap.find(Key); It != RegionLiveRegMap.end())
      return &It->getSecond();
    return nullptr;
  }
};
...
private:
  ...
  /// Whethre the stage will rollback its rematerialization post-rescheduling if
  /// they were useless or detrimental.
  bool SupportRollback;
  /// Indices of rematerialized registers to rollback if the stage does not end
  /// up being beneficial.
  ...
  void removeFromLiveMaps(unsigned RegIdx, Register DefReg,
...
public:
  ...
  PreRARematStage(GCNSchedStageID StageID, GCNScheduleDAGMILive &DAG)
      : GCNSchedStage(StageID, DAG), RescheduleRegions(DAG.Regions.size()),
        Remater(MF, DAG.Regions,
                /*RegionsTopDown=*/DAG.doMBBSchedRegionsTopDown(), *DAG.LIS) {}
};
