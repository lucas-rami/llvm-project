...
private:
  ...
  struct RematReg {
    ...
    MachineInstr *DefMI;
    /// Position at which the register shoyld be rematerialized.
    MachineBasicBlock::iterator InsertPoint;
    /// Regions in which rematerializing the register is guaranteed to decrease
    /// RP.
    BitVector Beneficial;
    /// Regions in which rematerializing the register is likely but not
    /// guaranteed to decrease RP.
    BitVector MaybeBeneficial;
    /// Always equals \ref Beneficial | \ref MaybeBeneficial. Represents all
    /// regions in which the rematerializable register is live.
    BitVector Live;
  ...
  /// A scored rematerializable register. The score is depends on the current
  /// state of the stage and can be updated on demand.
  struct ScoredRemat {
  ...
  GCNRegPressure computeRegionRP(unsigned I);

  /// Fully recomputes RP from the DAG in \p Regions. Among those, sets again
  /// all \ref TargetRegions that were optimistically marked as satisfied but
