...
bool PreRARematStage::canIncreaseOccupancyOrReduceSpill() {
  ...
  std::pair<unsigned, unsigned> WavesPerEU = ST.getWavesPerEU(MF.getFunction());
  ...
  for (unsigned I = 0, E = DAG.Regions.size(); I != E; ++I) {
    ...
    GCNRPTarget RPTarget(MF, DAG.Pressure[I], /*AGPRToArchVGPRSpill=*/false);
    if (IncreaseOccupancy) {
      if (!RPTarget.reached()) {
        ...
        OptRegions.clear();
      } else if (DAG.RegionsWithMinOcc[I]) {
        // There is no spilling in the region, try to increase occupancy.
        RPTarget.setTargetOccupancy(DAG.MinOccupancy + 1, ST,
                                    /*AGPRToArchVGPRSpill=*/false);
      }
    }
    if (!RPTarget.reached())
      OptRegions.insert({I, RPTarget});
  }
  if (OptRegions.empty())
    return false;
  ...
  auto ReduceRPInRegion = [&](auto OptIt, Register Reg, LaneBitmask Mask,
    ...
    Progress |= Target.saveRegIfUseful(Reg, Mask, DAG.MRI);
    if (Target.reached())
