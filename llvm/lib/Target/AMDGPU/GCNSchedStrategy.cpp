...
#if !defined(NDEBUG) || defined(LLVM_ENABLE_DUMP)
Printable PreRARematStage::Candidate::print() const {
...
bool PreRARematStage::initGCNSchedStage() {
  ...
  const Candidate::FreqInfo FreqInfo(MF, DAG);
  ...
  for (unsigned I = 0, E = DAG.MRI.getNumVirtRegs(); I != E; ++I) {
    ...
    if (DAG.MRI.hasOneDef(Reg))
      continue;
    dbgs() << "Printing def/use for " << llvm::printReg(Reg) << ":\n";
    for (MachineOperand &MO : DAG.MRI.def_operands(Reg)) {
      MachineInstr& DefMI = *MO.getParent();
      dbgs() << "  @" << DAG.LIS->getInstructionIndex(DefMI) << " def: " << DefMI;
    }
    for (MachineOperand &MO : DAG.MRI.use_operands(Reg)) {
      MachineInstr& DefMI = *MO.getParent();
      dbgs() << "  @" << DAG.LIS->getInstructionIndex(DefMI) << " use: " << DefMI;
    }
  }
  ...
  for (unsigned RegIdx = 0, E = Remater.getNumRegs(); RegIdx < E; ++RegIdx) {
    ...
    // We further filter the registers that we can rematerializer based on our
    ...
    Candidate &Cand = Candidates.emplace_back(RegIdx, FreqInfo, Remater, DAG);
  ...
  SmallVector<unsigned> NewCandidates;
  for (;;) {
    RecomputeRP.reset();
    ...
    REMAT_DEBUG({
      ...
      for (const Candidate &Cand : reverse(Candidates)) {
    ...
    while (!CandidateOrder.empty()) {
      const Candidate &Candidate = Candidates[CandidateOrder.back()];
      if (Candidate.isScoreStale(TargetRegions)) {
        REMAT_DEBUG(dbgs() << "Interrupt round on stale score for "
                           << Remater.printRematReg(Candidate.RegIdx));
        break;
      }
      CandidateOrder.pop_back();

      // Remove the register from all regions where it is a live-in or live-out.
      const Rematerializer::Reg &CandReg = Remater.getReg(Candidate.RegIdx);
      removeFromLiveMaps(CandReg.getDefReg(), CandReg.Mask, Candidate.LiveIn,
                         Candidate.LiveOut);

      REMAT_DEBUG(dbgs() << "** REMAT "
                         << Remater.printRematReg(Candidate.RegIdx) << '\n';);
      Candidate.rematerialize(Remater);
      ...
      updateRPTargets(Candidate.Live, Candidate.RPSave);
      RecomputeRP |= Candidate.ApproximateDiff;
      RescheduleRegions |= Candidate.Live;
    ...
    // We need up-to-date live intervals for re-computing pressure in target
    // regions.
    ...
    for (unsigned CandIdx : CandidateOrder) {
      Candidate &Cand = Candidates[CandIdx];
      if (Cand.update(TargetRegions, RPTargets, FreqInfo, !TargetOcc))
        NewCandidates.push_back(CandIdx);
    }
    if (NewCandidates.empty()) {
      REMAT_DEBUG(dbgs() << "Stop on exhausted rematerialization candidates\n");
      break;
    }
    CandidateOrder = std::move(NewCandidates);
  }
...
PreRARematStage::Candidate::FreqInfo::FreqInfo(
...
PreRARematStage::Candidate::Candidate(RegisterIdx RegIdx, const FreqInfo &Freq,
    ...
    : RegIdx(RegIdx), LiveIn(DAG.Regions.size()), LiveOut(DAG.Regions.size()),
      ...
      MaybeBeneficial(DAG.Regions.size()) {
...
bool PreRARematStage::Candidate::update(const BitVector &TargetRegions,
                                        ArrayRef<GCNRPTarget> RPTargets,
                                        const FreqInfo &FreqInfo,
                                        bool ReduceSpill) {
  MaybeBeneficial.reset();
  MaxFreq = 0;
  RegionImpact = 0;
  for (unsigned I : TargetRegions.set_bits()) {
    if (!Live[I])
    ...
    RegionImpact += (ApproximateDiff[I] ? 1 : 2) * NumRegsBenefit;

    if (ReduceSpill) {
      uint64_t Freq = FreqInfo.Regions[I];
      if (ApproximateDiff[I]) {
...
void PreRARematStage::Candidate::rematerialize(Rematerializer &Remater) const {
  const Rematerializer::Reg &Reg = Remater.getReg(RegIdx);
...
void PreRARematStage::removeFromLiveMaps(Register Reg, LaneBitmask Mask,
  ...
  RemovedFromLiveMaps.push_back({Reg, Mask, LiveIn, LiveOut});
...
void PreRARematStage::finalizeGCNSchedStage() {
  ...
  for (const auto &[Reg, Mask, LiveIn, LiveOut] : RemovedFromLiveMaps)
    addToLiveMaps(Reg, Mask, LiveIn, LiveOut);
