...
void GCNScheduleDAGMILive::updateRegionLiveOuts() {
  ...
  for (auto [I, Bounds] : enumerate(Regions)) {
    MachineInstr *LastMI = getLastMIForRegion(Bounds.first, Bounds.second);
...
void PreRARematStage::RematReg::print(
  ...
  for (const auto &[I, RegionUsers] : zip(UsedIn.set_bits(), Users)) {
    REMAT_DEBUG(dbgs() << "    [" << I << "] used " << RegionUsers.size()
                       << " time(s)\n");
    for (const MachineInstr *UserMI : RegionUsers)
      REMAT_DEBUG(dbgs() << "      -> " << *UserMI);
  }
...
bool PreRARematStage::initGCNSchedStage() {
  ...
  REMAT_DEBUG({
    ...
    if (TargetRegions.none()) {
      ...
      LLVM_DEBUG(dbgs() << "increase occupancy from " << *TargetOcc - 1
                        << "\n");
    } else {
  ...
  // TODO: Cannot directly build MBFI here because of lack of const in
  // constructor.
  ...
  while ((verifyRPTargets(RecomputeRP) || TargetRegions.any()) &&
    ...
    for (; RematIdx >= 0 && TargetRegions.any(); --RematIdx) {
      const RematReg &Remat = *ScoredRematInstrs[RematIdx].Remat;
      ...
      // such cases.
      if (Score <= 0) {
        REMAT_DEBUG(dbgs() << "Stop on non-positive score --> "
                           << *Remat.DefMI);
        NoMoreRemat = true;
        break;
      }
...
void PreRARematStage::setObjective() {
  ...
  // There is no spilling; set up "increased occupancy" targets for all regions.
...
PreRARematStage::RematReg::RematReg(
    MachineInstr *DefMI, ArrayRef<RegionUsers> Uses, GCNScheduleDAGMILive &DAG,
    const DenseMap<MachineInstr *, unsigned> &MIRegion,
    ArrayRef<uint64_t> RegionFreq)
    : DefMI(DefMI), LiveIn(DAG.Regions.size()), LiveOut(DAG.Regions.size()),
      Live(DAG.Regions.size()), UsedIn(DAG.Regions.size()),
      DefFrequency(RegionFreq[MIRegion.at(DefMI)]), UseFrequency(0) {
  assert(!Uses.empty() && "at least one using region");

  // Build data about the register's uses.
  for (const RegionUsers &RU : Uses) {
    assert(!RU.Users.empty() && "at least one use per region");
    Users.push_back(RU.Users);
    UseFrequency += RegionFreq[RU.Region];

    assert(UsedIn.find_last() != (int)RU.Region && "duplicate region");
    UsedIn.set(RU.Region);
    assert(UsedIn.find_last() == (int)RU.Region && "regions not in order");
  }
  ...
  for (unsigned I = 0, E = DAG.Regions.size(); I != E; ++I) {
    ...
    if (LiveInIt != DAG.LiveIns[I].end() && LiveInIt->second.any())
      LiveIn.set(I);
    auto LiveOutIt = DAG.LiveOuts[I].find(Reg);
    if (LiveOutIt != DAG.LiveOuts[I].end() && LiveOutIt->second.any())
      LiveOut.set(I);
  }
  Live |= LiveIn;
  Live |= LiveOut;
...
void PreRARematStage::ScoredRemat::update(const BitVector &TargetRegions,
  ...
  // reduce RP.
  if (ReduceSpill && InstrLatencyGain < 0) {
    if (!Remat->intersectWithTarget(TargetRegions)) {
  ...
  // The register's RP score is proportional to the total estimated number of
  // executions in target regions where the register is live.
  Register Reg = Remat->DefMI->getOperand(0).getReg();
  Score = 0;
  for (unsigned I : TargetRegions.set_bits()) {
    if (Remat->isBeneficialRegion(I))
      Score += WeightRP * RPTargets[I].isSaveBeneficial(Reg) * RegionFreq[I];
    else if (Remat->isMaybeBeneficialRegion(I))
      Score +=
          WeightRPMaybe * RPTargets[I].isSaveBeneficial(Reg) * RegionFreq[I];
  }

  // The score is directly proportional to the size of the rematerializable reg.
  Score *= SIRegisterInfo::getNumCoveredRegs(Remat->Mask);
}
...
PreRARematStage::rematerialize(const RematReg &Remat, BitVector &RecomputeRP) {
  ...
  for (const auto &[UseRegion, RegionUsers] :
       zip(Remat.UsedIn.set_bits(), Remat.Users)) {
...
void PreRARematStage::rollback(const RollbackReg &Rollback) const {
  ...
  // rematerializations.
  for (const auto &[RematMI, UseRegion, RegionUsers] :
       zip(RematMIs, Remat->UsedIn.set_bits(), Remat->Users)) {
    REMAT_DEBUG({
