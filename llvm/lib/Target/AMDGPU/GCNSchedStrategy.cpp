...
bool PreRARematStage::initGCNSchedStage() {
  ...
  for (unsigned I = 0; I < NumRegions; ++I) {
    ...
    // MI terminators are considered to be part of the region they delimitate.
  ...
  if (!computeRPTargets() || !collectRematInstrs())
    return false;

#ifndef NDEBUG
  if (IncreaseOcc) {
    REMAT_DEBUG(dbgs() << "Trying to increase occupancy over "
                       << DAG.MinOccupancy << "\n");
  } else {
    REMAT_DEBUG(
        dbgs() << "Trying to reduce spilling w.r.t. minimum target occupancy ("
               << ST.getWavesPerEU(MF.getFunction()).first << "):\n");
  }
#endif

  // Compute region frequencies. Cannot directly build MFI here because of lack
  // of const in constructor.
  // TODO how do we handle unknown frequencies (which will show up as 0)?
  ...
  for (const MachineBasicBlock *MBB : RegionBB)
    RegionFreq.push_back(MFI.getBlockFreq(MBB).getFrequency() /
                         MFI.getEntryFreq().getFrequency());

#ifndef NDEBUG
  REMAT_DEBUG(dbgs() << "Region frequencies:\n");
  for (auto [I, Freq] : llvm::enumerate(RegionFreq))
    REMAT_DEBUG(dbgs() << "  [" << I << "] " << Freq << '\n');
  REMAT_DEBUG(dbgs() << "Rematerializable instructions:\n");
  for (const RematInstruction &Remat : RematInstrs) {
    REMAT_DEBUG(dbgs() << "  [" << MIRegion.at(Remat.DefMI) << "] "
                       << *Remat.DefMI);
    REMAT_DEBUG(dbgs() << "    -> used in [" << MIRegion.at(&*Remat.InsertPoint)
                       << "] " << *Remat.InsertPoint);
    if (Remat.StrictBenefit.any()) {
      REMAT_DEBUG(dbgs() << "    beneficial in regions ");
      for (unsigned I : Remat.StrictBenefit.set_bits())
        LLVM_DEBUG(dbgs() << I << " ");
      LLVM_DEBUG(dbgs() << '\n');
    }
    if (Remat.PossibleBenefit.any()) {
      REMAT_DEBUG(dbgs() << "    possibly beneficial in regions ");
      for (unsigned I : Remat.PossibleBenefit.set_bits())
        LLVM_DEBUG(dbgs() << I << " ");
      LLVM_DEBUG(dbgs() << '\n');
    }
  }

  auto PrintTargetRegions = [&]() {
    if (TargetRegions.none()) {
      REMAT_DEBUG(dbgs() << "No more target regions!\n");
    } else {
  ...
  BitVector AlwaysBenefRegions(NumRegions);
  for (const RematInstruction &Remat : RematInstrs) {
    if (isAlwaysBeneficial(Remat)) {
      REMAT_DEBUG(dbgs() << "  REMAT (always) --> " << *Remat.DefMI);
      rematerialize(Remat, RecomputeRP);
      RescheduleRegions |= Remat.AllLiveRegions;
    } else {
      ScoredRematInstrs.emplace_back(&Remat, *this);
    }
  }
  UpdateTargetRegions(AlwaysBenefRegions);
  ...
  while ((verifyRPTargets(RecomputeRP) || TargetRegions.any()) &&
         !ScoredRematInstrs.empty()) {
    // (Re-)Score everything and (re-)sort in inceasing score order.
    for (ScoredRemat &Remat : ScoredRematInstrs)
      Remat.update(*this);
    llvm::stable_sort(ScoredRematInstrs);

#ifndef NDEBUG
    REMAT_DEBUG(dbgs() << "==== ROUND " << RoundNum << " ====\n");
    for (const ScoredRemat &SRemat : ScoredRematInstrs) {
      REMAT_DEBUG(dbgs() << "  *" << SRemat.getScore() << "* | "
                         << *SRemat.Remat->DefMI);
    }
#endif

    // Rematerialize instructions until we hit a roadblock or estimate that we
    // have reached the RP targets.
    RecomputeRP.clear();
    int RematIdx = ScoredRematInstrs.size() - 1;
    bool NoMoreRemat = false;
    for (; RematIdx >= 0 && TargetRegions.any(); --RematIdx) {
      const RematInstruction &Remat = *ScoredRematInstrs[RematIdx].Remat;
      int Score = ScoredRematInstrs[RematIdx].getScore();
      ...
      // such cases.
      if (Score <= 0) {
        REMAT_DEBUG(dbgs() << "  Stop on negative score --> " << *Remat.DefMI);
        NoMoreRemat = true;
        break;
      }

      // When previous rematerializations in this round have already achieved
      // the objective in all regions this rematerialization can impact, we have
      // a good indication that our scores have diverged significantly from
      // reality. Interrupt this round and re-score.
      if (!Remat.intersectWithTarget(TargetRegions)) {
        REMAT_DEBUG(dbgs() << "  Stop on stale score --> " << *Remat.DefMI);
        break;
      }
      ...
      if (IncreaseOcc) {
        // Every rematerialization done with the objective of increasing
        // occupancy increases latency. If we don't manage to increase
        // occupancy, we want to roll them back.
        Rollbackable.push_back({RematMI, &Remat});
      }
      UpdateTargetRegions(Remat.AllLiveRegions);
      RescheduleRegions |= Remat.AllLiveRegions;
    }
#ifndef NDEBUG
    PrintTargetRegions();
    ...
    // Peel off the instructions we already rematerialized from the vector's
    // tail.
    ScoredRematInstrs.truncate(RematIdx + 1);
  }
...
bool PreRARematStage::computeRPTargets() {
  ...
  for (unsigned I = 0, E = DAG.Regions.size(); I != E; ++I) {
    ...
    // point we err on the conservative side and do not increase reg-to-reg
    // spilling for the sake of increasing occupancy.
    const GCNRegPressure &RP = DAG.Pressure[I];
    GCNRPTarget &RPTarget =
        RPTargets.emplace_back(MaxSGPRsNoSpill, MaxVGPRsNoSpill, ST, RP,
    ...
    if (IncreaseOcc) {
      if (!RPTarget.satisfied()) {
        ...
        for (unsigned J = 0; J < I; ++J) {
          RPTargets[J] =
              GCNRPTarget(MaxSGPRsNoSpill, MaxVGPRsNoSpill, ST, DAG.Pressure[J],
                          /*AGPRToArchVGPRSpill=*/true);
        }
      } else {
        // There is no spilling in the region, try to increase occupancy.
        RPTarget = GCNRPTarget(MaxSGPRsIncOcc, MaxVGPRsIncOcc, ST, RP,
...
bool PreRARematStage::collectRematInstrs() {
  ...
  for (unsigned I = 0, E = NumRegions; I != E; ++I) {
    ...
    for (auto MI = Region.first; MI != Region.second; ++MI) {
      ...
      if ((UseFirstMO.isReg() && RematRegs.contains(UseFirstMO.getReg())) ||
      ...
      RematInstrs.emplace_back(&DefMI, *UseMI, DAG, MIRegion);
    }
  }

  return !RematInstrs.empty();
}

PreRARematStage::RematInstruction::RematInstruction(
    MachineInstr *DefMI, MachineBasicBlock::iterator InsertPoint,
    GCNScheduleDAGMILive &DAG,
    const DenseMap<MachineInstr *, unsigned> &MIRegion)
    : DefMI(DefMI), InsertPoint(InsertPoint), StrictBenefit(DAG.Regions.size()),
      PossibleBenefit(DAG.Regions.size()), AllLiveRegions(DAG.Regions.size()) {

  PossibleBenefit.set(MIRegion.at(DefMI));
  ...
  for (unsigned I = 0, E = DAG.Regions.size(); I != E; ++I) {
    ...
    if (LiveInIt == DAG.LiveIns[I].end() || LiveInIt->second.none())
      continue;

    const GCNRPTracker::LiveRegSet &LiveOuts =
        DAG.RegionLiveOuts.getLiveRegsForRegionIdx(I);
    auto LiveOutIt = LiveOuts.find(Reg);
    bool IsLiveOut = LiveOutIt != LiveOuts.end() && LiveOutIt->second.any();
    ...
    if (IsLiveOut && I != UseRegion)
      ...
      PossibleBenefit.set(I);
  }

  AllLiveRegions |= PossibleBenefit;
  AllLiveRegions |= StrictBenefit;
}

PreRARematStage::ScoredRemat::ScoredRemat(const RematInstruction *Remat,
...
void PreRARematStage::ScoredRemat::update(const PreRARematStage &Stage) {
  ...
  // The rematerialization's score is determined by the RP reduction we expect
  // it to bring in critical regions.
  ...
  // Each target region which may benefit from the rematerialization contributes
  // to the score.
  BitVector IntersectRegions = Stage.TargetRegions;
  IntersectRegions &= Remat->StrictBenefit;
  Score = WeightRPStrict * ComputeRegionsRPBenefit(IntersectRegions);
  IntersectRegions = Stage.TargetRegions;
  IntersectRegions &= Remat->PossibleBenefit;
  Score += WeightRPPossible * ComputeRegionsRPBenefit(IntersectRegions);

  // The score is directly proportional to the size of the rematerializable reg.
  Score *= SIRegisterInfo::getNumCoveredRegs(
      Stage.DAG.RegionLiveOuts.getLiveRegsForRegionIdx(DefRegion)[Reg]);
}

bool PreRARematStage::isAlwaysBeneficial(const RematInstruction &Remat) const {
...
MachineInstr *PreRARematStage::rematerialize(const RematInstruction &Remat,
  ...
  auto UseRegion = MIRegion.find(&*Remat.InsertPoint);
  if (UseRegion != MIRegion.end()) {
    DAG.updateRegionBoundaries(DAG.Regions[UseRegion->second],
                               Remat.InsertPoint, RematMI);
  }
  DAG.LIS->RemoveMachineInstrFromMaps(DefMI);
  DefMI.eraseFromParent();

  auto RemoveFromLiveIns = [&](unsigned I) {
    ...
    // contains the single use.
    LaneBitmask PrevMask = RegionLiveIns[Reg];
    RegionLiveIns.erase(Reg);
    RegMasks.insert({{I, RematMI->getOperand(0).getReg()}, PrevMask});
  };

  // Remove the register from all regions where it is a live-in and reduce RP.
  for (unsigned I : Remat.AllLiveRegions.set_bits()) {
    // This save is exact in live-through regions where the register is not used
    // but optimistic in all other regions where the register is live.
    RPTargets[I].saveReg(Reg, DAG.LiveIns[I][Reg], DAG.MRI);
    if (I != DefRegion)
      RemoveFromLiveIns(I);
  }

  // Regions where RP changes unpredictably must have their RP fully recomputed.
  RecomputeRP |= Remat.PossibleBenefit;
...
void PreRARematStage::rollback() const {
  ...
  for (const auto &[RematMI, Remat] : Rollbackable) {
    unsigned DefRegion = MIRegion.at(Remat->DefMI);
    ...
    BitVector AllLiveRegions(Remat->StrictBenefit);
    AllLiveRegions |= Remat->PossibleBenefit;
    for (unsigned I : AllLiveRegions.set_bits()) {
      if (I != DefRegion)
        DAG.LiveIns[I].insert({Reg, RegMasks.at({I, Reg})});
    }
  }
}

GCNRegPressure PreRARematStage::recomputeRegionRP(unsigned I) {
  bool IsEmptyRegion = DAG.Regions[I].first == DAG.Regions[I].second;
  if (IsEmptyRegion)
    return getRegPressure(DAG.MRI, DAG.LiveIns[I]);
  ...
  return RPT.moveMaxPressure();
}

bool PreRARematStage::verifyRPTargets(const BitVector &Regions) {
  bool TooOptimistic = false;
  for (unsigned I : Regions.set_bits()) {
    GCNRPTarget &Target = RPTargets[I];
    Target.setPressure(recomputeRegionRP(I));

    // Since we were optimistic in assessing RP decreases in these regions, we
    // may need to remark the target as a target region if RP didn't decrease as
    // expected.
    if (!TargetRegions[I] && !Target.satisfied()) {
      REMAT_DEBUG(dbgs() << "[" << I
                         << "] Overly optimistic RP estimation in region, "
                            "remarking as target\n");
      TooOptimistic = true;
      TargetRegions.set(I);
    }
...
void PreRARematStage::finalizeGCNSchedStage() {
  ...
  for (auto &[_, Remat] : Rollbackable)
    ImpactedRegions |= Remat->AllLiveRegions;
  for (unsigned I : ImpactedRegions.set_bits())
    DAG.Pressure[I] = recomputeRegionRP(I);
