...
bool PreRARematStage::initGCNSchedStage() {
  ...
  SupportRollback = TargetOcc && !DisablePreRARematRollback;
  performInterRegionRemats(RI);
...
std::optional<unsigned> PreRARematStage::RegionsInfo::determineObjective(
  ...
  if (HasVectorRegisterExcess || DAG.MinOccupancy >= DAG.MFI.getMaxWavesPerEU())
    return std::nullopt;
...
void PreRARematStage::RegionsInfo::calculateRanges(
  ...
  const GCNRegPressure &MaxRP = Target.getCurrentRP();
...
struct InterRegionCand {
  ...
  unsigned RootIdx;
  /// Cache the defined register (to remember it if the candidate is fully
  /// rematerialized).
  Register DefReg;
  ...
  InterRegionCand(unsigned RootIdx, const GCNScheduleDAGMILive &DAG,
                  const Rematerializer &Remater, bool SkipLiveCheck = false);
  ...
  void rematerialize(bool SupportRollback, const GCNScheduleDAGMILive &DAG,
  ...
  bool update(bool ReduceSpill, const PreRARematStage::RegionsInfo &RI,
              const PreRARematStage::FreqInfo &FI,
              const Rematerializer &Remater, const LiveIntervals &LIS);

  /// Whether the current score is considered stale enough w.r.t. the current
  /// target regions to make the rematerialization decision potentially not
  /// helpful at all.
  bool isScoreStale(const PreRARematStage::RegionsInfo &RI);

  /// Whether rematerializing the candidate is considered to be actively
  /// detrimental to performance.
  bool isDetrimental(const PreRARematStage::RegionsInfo &RI,
                     const Rematerializer &Remater,
                     const LiveIntervals &LIS) const;

  /// Determines whether rematerializing register the candidate will help in
  /// reducing RP in region \p RegionIdx.
  bool isBeneficialIn(unsigned RegionIdx,
                      const PreRARematStage::RegionsInfo &RI,
                      const Rematerializer &Remater,
                      const LiveIntervals &LIS) const;
...
private:
  ...
  void makeRematPlan(const Rematerializer &Remater, const LiveIntervals &LIS);
...
InterRegionCand::InterRegionCand(unsigned RootIdx,
                                 const GCNScheduleDAGMILive &DAG,
                                 const Rematerializer &Remater,
                                 bool SkipLiveCheck)
    : RootIdx(RootIdx), LiveIn(Remater.getNumRegions()),
      LiveOut(Remater.getNumRegions()), Live(Remater.getNumRegions()),
      ApproximateDiff(Remater.getNumRegions()),
      BeneficialRegions(Remater.getNumRegions()) {
...
void InterRegionCand::rematerialize(bool SupportRollback,
                                    const GCNScheduleDAGMILive &DAG,
                                    Rematerializer &Remater) {
  REMAT_DEBUG(dbgs() << "** REMAT " << Remater.printID(RootIdx) << " to "
                     << Remats.size() << " region(s)\n");
  for (auto &[UseRegion, RDI] : Remats) {
    Remater.rematerializeToRegion(RootIdx, UseRegion, SupportRollback, RDI.DRI);
    REMAT_DEBUG({
      ...
      for (unsigned I = E - RDI.DepRemats.size(); I < E; ++I) {
        dbgs() << REMAT_PREFIX << "  "
               << Remater.printID(Remater.getParentOf(I)) << " -> "
               << Remater.printID(I) << '\n';
      }
    });
  }
...
bool InterRegionCand::update(bool ReduceSpill,
                             const PreRARematStage::RegionsInfo &RI,
                             const PreRARematStage::FreqInfo &FI,
                             const Rematerializer &Remater,
                             const LiveIntervals &LIS) {
  BeneficialRegions.reset();
  makeRematPlan(Remater, LIS);
  if (Remats.empty())
    return false;
  ...
  FreqDiff = RootReg.hasUsersInDefRegion()
                 ? 0
                 : std::max(FI.Regions[RootReg.DefRegion], (uint64_t)1);
  for (const auto &[UseRegion, RRI] : Remats) {
  ...
  for (unsigned RegionIdx : RI.TargetRegions.set_bits()) {
    if (!isBeneficialIn(RegionIdx, RI, Remater, LIS))
      continue;
    ...
    if (ReduceSpill) {
      if (RootReg.Uses.contains(RegionIdx))
        UnusedInTargetRegions = false;
      uint64_t RegionFreq = FI.Regions[RegionIdx];
      MaxFreq =
          std::max(MaxFreq, (ApproximateDiff[RegionIdx] ? 1 : 2) * RegionFreq);
    }
  }
...
bool InterRegionCand::isDetrimental(const PreRARematStage::RegionsInfo &RI,
                                    const Rematerializer &Remater,
                                    const LiveIntervals &LIS) const {
  ...
  const Rematerializer::Reg &Reg = Remater.getReg(RootIdx);
  for (const auto &[UseRegion, RRI] : Remats) {
    if (RRI.DepRemats.empty() || !RI.TargetRegions[UseRegion])
      continue;
    ...
    MachineInstr *FirstUser = Reg.getRegionUseBounds(UseRegion, LIS).first;
    SlotIndex RematSlot = LIS.getInstructionIndex(*FirstUser);
...
bool InterRegionCand::isScoreStale(const PreRARematStage::RegionsInfo &RI) {
  ...
  return !RI.TargetRegions.anyCommon(BeneficialRegions);
}

bool InterRegionCand::isBeneficialIn(unsigned RegionIdx,
                                     const PreRARematStage::RegionsInfo &RI,
                                     const Rematerializer &Remater,
                                     const LiveIntervals &LIS) const {
  assert(RI.TargetRegions[RegionIdx] && "not a target region");
  if (!Live[RegionIdx])
    return false;
  ...
  const Rematerializer::Reg &Reg = Remater.getReg(RootIdx);
  assert(Reg.hasUsersOutsideDefRegion() && "useless rematerialization");
  if (Reg.DefRegion == RegionIdx) {
    // In the defining region, rematerializating the register will
    // reduce RP from the last use slot to the end of the region. If the
    // register has no users in its defining region, RP will be reduced from its
    // def slot instead. In both cases RP will be reduced at live-outs.
    if (!RI.satisfiedAtLiveOut(RegionIdx))
      return true;

    MachineInstr *LastUser = Reg.getRegionUseBounds(RegionIdx, LIS).second;
    SlotIndex LastSlot =
        Reg.hasUsersInDefRegion()
            ? LIS.getInstructionIndex(*LastUser).getRegSlot(true)
            : LIS.getInstructionIndex(*Reg.DefMI).getRegSlot();
    return LastSlot <= RI.getTargetRanges(RegionIdx).back().second;
  }
  assert(LiveIn[RegionIdx] && "expected register to be live-in");
  ...
  if (!RI.satisfiedAtLiveIn(RegionIdx))
    return true;
  const auto &Uses = Reg.Uses.find(RegionIdx);
  if (LiveOut[RegionIdx] &&
      (!RI.satisfiedAtLiveOut(RegionIdx) || Uses == Reg.Uses.end()))
    return true;

  // The register will no longer be live before its first use in the
  // region.
  assert(Uses != Reg.Uses.end() && "expected register uses in region");
  auto [FirstUser, LastUser] = Reg.getRegionUseBounds(RegionIdx, LIS);
  SlotIndex FirstUse = LIS.getInstructionIndex(*FirstUser).getRegSlot(true);
  if (RI.getTargetRanges(RegionIdx).front().first < FirstUse)
    return true;
  ...
  if (LiveOut[RegionIdx]) {
    ...
    SlotIndex LastUse = LIS.getInstructionIndex(*LastUser).getRegSlot(true);
    if (LastUse <= RI.getTargetRanges(RegionIdx).back().second)
      return true;
  }
...
bool InterRegionCand::isRematPlanValid(
    const Rematerializer &Remater,
    const DenseSet<unsigned> &ChangedUsers) const {
...
bool InterRegionCand::operator<(const InterRegionCand &O) const {
...
void InterRegionCand::makeRematPlan(const Rematerializer &Remater,
                                    const LiveIntervals &LIS) {
  Remats.clear();
  LRExtensions.clear();
  const Rematerializer::Reg &RootReg = Remater.getReg(RootIdx);
...
void InterRegionCand::computeRegionDependencies(unsigned UseRegion,
                                                SlotIndex Use,
                                                const Rematerializer &Remater,
                                                const LiveIntervals &LIS) {
  ...
  Rematerializer::DependencyReuseInfo &DRI = Remats[UseRegion].DRI;
  do {
    ...
    // possible.
    const Rematerializer::Reg &ChainReg = Remater.getReg(ChainRegIdx);
    for (unsigned MOIdx : Remater.getUnrematableOprds(ChainRegIdx)) {
      const MachineOperand &MO = ChainReg.DefMI->getOperand(MOIdx);
      if (!Remater.isMOAvailableAtUses(MO, Use))
    ...
    for (const Rematerializer::Reg::Dependency &Dep : ChainReg.Dependencies) {
      if (!SeenDeps.insert(Dep.RegIdx).second)
        continue;
      if (Remater.isMOAvailableAtUses(ChainReg.DefMI->getOperand(Dep.MOIdx),
                                      Use)) {
        ...
        if (RematIdx != Rematerializer::NoReg) {
          ...
          // register.
          DRI.useRemat(Dep.RegIdx, RematIdx);
        } else {
...
bool PreRARematStage::performInterRegionRemats(RegionsInfo &RI) {
  SmallVector<InterRegionCand, 4> Candidates;
  auto CreateCandidates = [&](bool SkipLiveChecks) -> void {
    ...
    for (unsigned RegIdx = Candidates.size(); RegIdx < NumRegs; ++RegIdx) {
      Candidates.emplace_back(RegIdx, DAG, Remater, SkipLiveChecks);
      REMAT_DEBUG(dbgs() << "  " << Remater.printRematReg(RegIdx) << '\n');
    }
  };
  ...
  for (;;) {
    ...
    for (InterRegionCand &Cand : Candidates) {
      if (Cand.update(ReduceSpill, RI, Freq, Remater, *DAG.LIS))
    ...
    while (!CandidateOrder.empty()) {
      ...
      if (Cand.isDetrimental(RI, Remater, *DAG.LIS)) {
        REMAT_DEBUG(dbgs() << "** SKIP " << Remater.printRematReg(Cand.RootIdx)
                           << "\n");
        continue;
      }

      Cand.rematerialize(SupportRollback, DAG, Remater);
      if (SupportRollback)
...
bool PreRARematStage::performIntraRegionRemats(RegionsInfo &RI) {
  ...
  for (unsigned RegIdx = 0, E = Candidates.size(); RegIdx < E; ++RegIdx) {
    ...
    if (!Reg.DefMI || Reg.DefMI->mayLoad() ||
      ...
      continue;
    const auto &[Region, DefUsers] = *Reg.Uses.begin();
    if (Region != Reg.DefRegion || DefUsers.size() < 2)
      continue;

    if (!RI.TargetRegions[Reg.DefRegion])
      continue;

    SmallVectorImpl<IntraRegionCand> &RegionCands = Candidates[Reg.DefRegion];
    IntraRegionCand &Cand = RegionCands.emplace_back(RegIdx, ST, DAG, Remater);
    ...
    ArrayRef<std::pair<SlotIndex, SlotIndex>> DelimSlots =
        RI.RegionRanges[Reg.DefRegion];
    SlotIndex DefSlot =
        DAG.LIS->getInstructionIndex(*Reg.DefMI).getRegSlot(true);
    SlotIndex RematBoundary = DelimSlots.back().second;
    ...
    MachineInstr *FirstUser = Reg.getRegionUseBounds(Region, *DAG.LIS).first;
    if (RematBoundary <= DAG.LIS->getInstructionIndex(*FirstUser)) {
      RegionCands.pop_back();
      continue;
    }

    REMAT_DEBUG(dbgs() << Remater.printID(RegIdx)
    ...
    for (MachineInstr *UseMI : DefUsers) {
    ...
    while (!CandidateOrder.empty()) {
      ...
      for (ArrayRef<MachineInstr *> Users : Cand.RematGroups) {
        REMAT_DEBUG(dbgs() << "  Rematerializing "
                           << Remater.printID(Cand.RegIdx) << " to "
                           << Remater.printUser(Users.front()) << '\n');
        // Remater.rematerializeTo(Cand.RegIdx, Users.front(), Users, {});
      }
      if (!Cand.RemainingUsers.empty()) {
        REMAT_DEBUG(dbgs() << "  Rematerializing "
                           << Remater.printID(Cand.RegIdx) << " to "
                           << Remater.printUser(&*Cand.InsertPos) << '\n');
        // Remater.rematerializeTo(Cand.RegIdx, Cand.InsertPos,
        // Cand.RemainingUsers,
        //                      {});
      }
...
void PreRARematStage::finalizeGCNSchedStage() {
  ...
  for (unsigned RegIdx : reverse(Rollbacks)) {
    REMAT_DEBUG(dbgs() << "** ROLLBACK " << Remater.printID(RegIdx) << '\n');
    Remater.rollbackRematsOf(RegIdx);
  }
  BitVector RecomputeRP(DAG.Regions.size());
  for (const auto &[RegIdx, Liveness] : RemovedFromLiveMaps) {
    addToLiveMaps(RegIdx, Liveness.first, Liveness.second);
    RecomputeRP |= Liveness.first;
    RecomputeRP |= Liveness.second;
  }
  Remater.updateLiveIntervals();
