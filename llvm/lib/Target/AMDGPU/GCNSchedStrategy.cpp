...
bool PreRARematStage::initGCNSchedStage() {
  ...
  if (HasVectorRegisterExcess) {
    ...
    SupportRollback = !DisablePreRARematRollback;
  ...
  Strategy = std::make_unique<RematAcrossForRPTarget>(
      MF, DAG.Regions, *DAG.LIS, MaxSGPRs, MaxVGPRs, SupportRollback, *DAG.MLI,
      DAG.LiveIns, LiveOuts);

  Strategy->performRematerializations(RescheduleRegions,
                                      !TargetOcc.has_value());
  if (RescheduleRegions.none())
    return false;

  // Set new maximum RP in all affected regions.
  LLVM_DEBUG(dbgs() << "==== REMAT RESULTS ====\n");
  unsigned DynamicVGPRBlockSize = MFI.getDynamicVGPRBlockSize();
  for (unsigned I : RescheduleRegions.set_bits()) {
    DAG.Pressure[I] = Strategy->getRegionPressure(I);
    LLVM_DEBUG(dbgs() << '[' << I << "] Achieved occupancy "
                      << DAG.Pressure[I].getOccupancy(ST, DynamicVGPRBlockSize)
                      << " (" << print(Strategy->getRegionPressure(I))
                      << ")\n");
  }
  ...
  LLVM_DEBUG({
    dbgs() << "Retrying function scheduling with new min. occupancy of "
           ...
           << DAG.MinOccupancy << ", target was " << getStageTargetOccupancy()
           << ")\n";
  });
...
bool PreRARematStage::initGCNRegion() {
  return !StopRescheduling && RescheduleRegions[RegionIdx] &&
...
void PreRARematStage::finalizeGCNSchedStage() {
  ...
  if (DAG.MinOccupancy >= *TargetOcc) {
    Strategy->commitRematerializations();
  ...
  Strategy->rollbackRematerializations();
  for (unsigned I : RescheduleRegions.set_bits())
