...
void PreRARematStage::ScoredRemat::update(const BitVector &TargetRegions,
  ...
  if (ReduceSpill) {
    // The register won't be spilled if its live range doesn't intersect with
    // target regions.
    if (!Remat->intersectWithTarget(TargetRegions))
      return setUselessRemat();

    // FIXME: We are just letting this through if we don't know the
    // instruction's latency gain because
    // (1) we don't know what to compare the spill latency,
    // (2) we don't want to miss rematerialization opportunities, and
    // (3) rematerializing is in most cases cheaper than spilling.
    // Ideally, such instruction should still be penalized with respect to
    // instructions for which we know the potential latency gain, all other
    // things being equal.
    if (InstrLatencyGain && *InstrLatencyGain < 0) {
      // It may be better to let the register spill if it is defined by a very
      // high latency instruction. Estimate the latency gain induced by
  ...
  // The register's estimated RP reduction is proportional to the total
  // estimated number of executions in target regions where the register is
  // live.
