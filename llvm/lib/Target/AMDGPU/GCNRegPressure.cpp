...
void GCNRegPressure::inc(unsigned Reg,
  ...
  if (TRI->getRegSizeInBits(*RC) != 32) {
    // Reg is from a tuple register class, account for the class's weight.
    if (PrevMask.none())
      TupleValue[RegKind] += Sign * TRI->getRegClassWeight(RC).RegWeight;
...
Printable llvm::print(const GCNRegPressure &RP, const GCNSubtarget *ST) {
  return Printable([&RP, ST](raw_ostream &OS) {
    OS << "VGPRs: " << RP.Value[GCNRegPressure::VGPR] << ' '
       << "AGPRs: " << RP.getAGPRNum();
    if (ST)
      OS << "(O"
