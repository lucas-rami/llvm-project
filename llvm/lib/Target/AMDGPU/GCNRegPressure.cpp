...
GCNRPTarget::GCNRPTarget(const MachineFunction &MF, const GCNRegPressure &RP,
    ...
    : RP(RP), AGPRToArchVGPRSpill(AGPRToArchVGPRSpill) {
...
GCNRPTarget::GCNRPTarget(unsigned NumSGPRs, unsigned NumVGPRs,
                         const GCNSubtarget &ST, const GCNRegPressure &RP,
                         bool AGPRToArchVGPRSpill)
    : RP(RP), AGPRToArchVGPRSpill(AGPRToArchVGPRSpill) {
  setRegLimits(NumSGPRs, NumVGPRs, ST);
}

GCNRPTarget::GCNRPTarget(unsigned Occupancy, const GCNSubtarget &ST,
                         const GCNRegPressure &RP, bool AGPRToArchVGPRSpill)
    : GCNRPTarget(ST.getMaxNumSGPRs(Occupancy, false),
                  ST.getMaxNumVGPRs(Occupancy), ST, RP, AGPRToArchVGPRSpill) {}
...
bool GCNRPTarget::satisfied() const {
  ...
  unsigned AGPRNum = RP.getAGPRNum();
  if (AGPRToArchVGPRSpill)
    AGPRNum -= std::min(AGPRNum, MaxVGPRs - RP.getArchVGPRNum());
  return AGPRNum <= MaxVGPRs;
}
