...
/// order of sub-definitions (for example during re-scheduling) because the
...
...
  ...
  auto MoveMIAndAdjustLiveness = [&](MachineInstr &MI) {
    // Replicates the scheduler's effect on LIS on an intra-block move.
    LIS.handleMove(MI);
  ...
  ASSERT_NE(Sub0Def.getOperand(0), Sub1Def.getOperand(0));
