...
/// when registers are deleted their respective integer handle and underlying
/// data storage remain valid. Method which perform actual rematerializations do
/// however invalidate addresses to \ref Rematerializer::Reg objects.
///
...
public:
  ...
  /// dependencies which are not declared in this struct will be rematerialized
  /// along the root.
  struct DependencyReuseInfo {
    /// Maps registers that the root transitively depends on to their
    /// rematerialization to use for the rematerialization of the root.
  ...
  /// DRI. If \p SupportRollback is set and the original register no longer has
  /// any user after the transfer, the rematerialization can later be rolled
  /// back.
  ///
  ...
  /// ToRegIdx, the latter of which must be a rematerialization of the former.
  /// Users in \p UseRegion must be reachable from \p ToRegIdx.
  ...
  /// the latter of which must be a rematerialization of the former. \p UserMI
  /// must be a direct user of \p FromRegIdx. \p UserMI must be reachable from
  /// \p ToRegIdx.
  void transferUser(unsigned FromRegIdx, unsigned ToRegIdx,
                    bool SupportRollback, MachineInstr &UserMI);
...
private:
  ...
  /// Fully rematerialized registers that are currently rollback-able. Values
  /// map register machine operand indices to their original register.
  ...
  /// SupportRollback is set, do not delete the defining instruction and only
  /// change its opcode to a debug one to remove it from consideration. This
  /// allows to rollback the deletion later on.
